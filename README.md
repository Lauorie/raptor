...
其中：
- $N$ 是文本中 token 的总数。
- $P(w_i \mid w_1, \dots, w_{i-1})$ 是模型对第 $i$ 个 token 的预测概率。

#### **直观解释**：
- 困惑度可以看作是模型在每一步预测中“平均面临多少种选择”的度量。
- 如果模型很确定地预测每个 token（概率 $P(w_i\mid...)$ 高），困惑度会很低。
- 如果模型对每个 token 的预测很不确定（概率 $P(w_i\mid...)$ 接近均匀分布），困惑度会很高。
...
3. **与预测概率的关系**：
   - 信息量和困惑度都与预测概率 $P(w_i \mid w_1, \dots, w_{i-1})$ 成反比（概率越高，信息量越低，困惑度越小）。
   - 信息量是单个事件的度量，而困惑度是多个事件（整个文本）上的综合度量。

#### **数学关系总结**：
- 信息量（单个 token）：$$ I(w_i) = -\log P(w_i \mid w_1, \dots, w_{i-1}) $$
- 平均信息量（NLL）：$\text{NLL} = \frac{1}{N} \sum_{i=1}^N -\log P(w_i \mid w_1, \dots, w_{i-1})$
- 困惑度：$PPL = \exp(\text{NLL})$
...
2. 计算概率:
   ...
   - $P(\text{"The"} \mid <start>) = 0.2$
   - $P(\text{"cat"} \mid \text{"The"}) = 0.1$
   - $P(\text{"sat"} \mid \text{"The cat"}) = 0.3$
   - $P(\text{"on"} \mid \text{"The cat sat"}) = 0.4$
   - $P(\text{"the"} \mid \text{"The cat sat on"}) = 0.5$
   - $P(\text{"mat"} \mid \text{"The cat sat on the"}) = 0.2$
...
4. 计算平均负对数似然:
   $(1.61 + 2.30 + 1.20 + 0.92 + 0.69 + 1.61) / 6 \approx 1.39$

5. 计算困惑度:
   $$
   \text{perplexity} = e^{\text{平均负对数似然}} = e^{1.39} \approx 4.01
   $$

所以,这个模型在这个简单句子上的困惑度约为 4.01。

解释:
- 困惑度可以被理解为模型在每个位置上平均需要从多少个选项中猜测。
- 困惑度越低,表示模型的预测越准确。在这个例子中,4.01 意味着模型在每个位置上平均需要从约 4 个选项中选择。
- 理想情况下,一个完美的模型会在每个位置都 100% 确定下一个词,这样困惑度就会是 1。
